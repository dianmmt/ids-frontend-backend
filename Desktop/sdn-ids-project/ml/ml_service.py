...
def load_model_from_bytes(content: bytes, fmt: str):
    """Load model from bytes according to format (pkl/h5)."""
    global model
    try:
        if fmt == 'pkl':
...
def health_check():
    ...
    return jsonify({
        ...
        'active_model': active_model_meta,
...
def model_load():
    """Load a new model from base64 content pushed by backend."""
    try:
        ...
        b64 = body.get('base64Content')

        if not name or not version or fmt not in ('pkl','h5') or not b64:
            return jsonify({'success': False, 'message': 'Invalid payload'}), 400

        import base64, hashlib
        content = base64.b64decode(b64)
        calc = hashlib.sha256(content).hexdigest()
        if sha256 and calc != sha256:
            return jsonify({'success': False, 'message': 'SHA256 mismatch'}), 400

        ok = load_model_from_bytes(content, fmt)
        if not ok:
            return jsonify({'success': False, 'message': 'Failed to load model'}), 500

        active_model_meta.update({
            'name': name,
            'version': version,
            'format': fmt,
            'framework': framework,
            'sha256': sha256 or calc
        })

        return jsonify({'success': True, 'message': 'Model loaded', 'active_model': active_model_meta})
    except Exception as e:
...
def initialize_app():
    ...
    print("✅ ML Server initialized successfully! (waiting for model load)")
