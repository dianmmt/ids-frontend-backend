...
-- ============================================================================
-- CONSOLIDATED: keep single metrics table used by backend
...
-- CONSOLIDATION: remove redundant performance tables and keep the ones actually used by backend services
-- Dropped: system_metrics, performance_history view/trigger chain in favor of performance_metrics + performance_alerts
-- Keep minimal operational performance-related tables referenced by code

-- Performance Alerts Table (used by PerformanceService + routes)
...
-- ML Processing Performance Table (used by PerformanceService)
CREATE TABLE IF NOT EXISTS ml_performance (
    id SERIAL PRIMARY KEY,
    inference_speed INTEGER NOT NULL,
    model_accuracy DECIMAL(5, 2) NOT NULL,
    processing_latency DECIMAL(10, 3) NOT NULL,
    queue_size INTEGER NOT NULL,
    processed_today BIGINT NOT NULL,
    timestamp TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_ml_performance_timestamp ON ml_performance(timestamp);

-- Database Performance Table (used by PerformanceService)
CREATE TABLE IF NOT EXISTS database_performance (
    id SERIAL PRIMARY KEY,
    active_connections INTEGER NOT NULL,
    max_connections INTEGER NOT NULL,
    avg_query_time DECIMAL(10, 3) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_database_performance_timestamp ON database_performance(timestamp);

-- Network Statistics Table (used by PerformanceService)
CREATE TABLE IF NOT EXISTS network_statistics (
    id SERIAL PRIMARY KEY,
    packets_per_second INTEGER NOT NULL,
    bandwidth_used BIGINT NOT NULL,
    dropped_packets_rate DECIMAL(5, 4) NOT NULL,
...
-- Removed obsolete performance history trigger and system_metrics functions in favor of simplified metrics retrieval on the backend
-- ============================================================================
