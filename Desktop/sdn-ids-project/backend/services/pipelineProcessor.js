...
export class PipelineProcessor {
  ...
  async processSingleFlow(flow) {
    try {
      ...
      // Get ML prediction
      const prediction = await this.mlPredictor.predictAttack(mlInput);
  ...
  prepareMLInput(flow) {
    return {
      ...
      byte_count: flow.total_length_of_fwd_packets + flow.total_length_of_bwd_packets,
      duration: flow.flow_duration,
      packets_per_second: flow.flow_packets_per_second,
      bytes_per_second: flow.flow_bytes_per_second,
      avg_packet_size: flow.average_packet_size,
      timestamp: flow.flow_start_time,
      
      // Additional CICFlowMeter features
      fwd_packet_length_mean: flow.fwd_packet_length_mean,
      bwd_packet_length_mean: flow.bwd_packet_length_mean,
    ...
    try {
      const queryText = `
        INSERT INTO attacks (
          ...
          model_name, model_version, inference_time_ms, detected_at
      ...
      console.log(`[PipelineProcessor] Attack detected: ${prediction.attackType} (${prediction.severity}) from ${flow.src_ip} to ${flow.dst_ip}`);
